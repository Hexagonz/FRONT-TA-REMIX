...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const defaultHari = ["Senin", "Selasa", "Rabu", "Kamis", "Jumat"].find(
  (h) => h.toLowerCase() === hariParam?.toLowerCase()
);
...
export default function AddJadwal() {
  const { defaultHari, guruList, kelasList, ruangList, canAddJadwal } =
  ...
  useEffect(() => {
    if (selectedKelasId) {
      form.setValue("id_kelas", String(selectedKelasId));
    }
    if (selectedRuangId) {
      form.setValue("id_ruang", String(selectedRuangId));
    }
  }, [selectedKelasId, selectedRuangId, form]);
            ...
            render={({ field }) => (
              <FormItem>
                ...
                >
                  <FormControl>
                    <SelectTrigger
                      className={kelasParam ? "pointer-events-none opacity-50" : ""}
                    >
