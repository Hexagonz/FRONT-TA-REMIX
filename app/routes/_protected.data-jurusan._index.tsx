...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  try {
    ...
    return json(
      {
        status: false,
        message: err.response?.data || "Terjadi kesalahan saat fetch jurusan",
...
export default function Index() {
  ...
  useEffect(() => {
    ...
    if (data) {
      toast.success(
        data === "1"
          ? "Sukses Menambahkan Jurusan Baru"
          : data === "2"
          ? "Sukses Mengedit Data Jurusan"
          : "Sukses Menghapus Data Jurusan",
        {
          position: "top-center",
          autoClose: 2997,
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      navigate("/data-jurusan?success=3", { replace: true });
    } catch (error) {
  ...
  return (
    <div className="*:mx-2">
      <Navbar title="Kelola Data Jurusan" />
            ...
            {filteredjurusans.map((jurusan: any) => (
              <TableRow key={jurusan.id_jurusan}>
                ...
                <TableCell className="text-right  w-max *:w-8 *:h-8 *:mx-1">
                  ...
                  >
                    <Link to={`/data-jurusan/view/${jurusan.id_jurusan}`}>
                  ...
                  >
                    <Link to={`/data-jurusan/${jurusan.id_jurusan}`}>
                  ...
                  <AlertComponent
                    ...
                    onClick={() => handleDelete?.(jurusan.id_jurusan)}
                    color="#FB2C36"
                  />
                </TableCell>
              </TableRow>
