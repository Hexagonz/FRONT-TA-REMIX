...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const ruangParam = url.searchParams.get("ruang");
  const jurusanParam = url.searchParams.get("jurusan");
  ...
  const ruangList = ruangRes.data.data;

  const selectedKelas = kelasList.find(
    (k: any) => k.kelas_romawi.toLowerCase() === kelasParam?.toLowerCase()
  );

  const selectedRuang = ruangList.find(
    (r: any) =>
      String(r.nomor_ruang) === ruangParam &&
      r.jurusan.nama_jurusan.toLowerCase() === jurusanParam?.toLowerCase()
  );

  const canAddJadwal = !!selectedKelas && !!selectedRuang;
...
export default function AddJadwal() {
  const { guruList, kelasList, ruangList, canAddJadwal } =
  ...
  const jurusanParam = searchParams.get("jurusan");

  const selectedKelasId = kelasList.find(
    (k: any) => k.kelas_romawi.toLowerCase() === kelasParam?.toLowerCase()
  )?.id_kelas;

  const selectedRuangId = ruangList.find(
    (r: any) =>
      String(r.nomor_ruang).toLowerCase() === ruangParam &&
      String(r.jurusan.nama_jurusan.toLowerCase()) === jurusanParam
  )?.id_ruang;
  ...
  const form = useForm<z.infer<typeof addSchema>>({
    ...
    defaultValues: {
      id_guru: "",
      id_kelas: selectedKelasId ? String(selectedKelasId) : "",
      id_ruang: selectedRuangId ? String(selectedRuangId) : "",
      hari: undefined,
      jam_mulai: "",
      jam_selesai: "",
    },
  ...
  if (!canAddJadwal) {
    return (
      <div className="text-center mt-10 text-red-500 font-semibold">
        Data kelas atau ruang tidak ditemukan. Tidak bisa menambah jadwal.
      </div>
    );
  }
  return (
      ...
      <Form {...form}>
        ...
        >
          ...
          <FormField
            ...
            name="id_kelas"
            render={({ field }) => {
              const isDisabled = !!kelasParam;

              const selectedKelasId = kelasList.find(
                (kelas: any) => kelas.kelas_romawi === kelasParam
              )?.id_kelas;
          ...
          <FormField
            ...
            name="id_ruang"
            render={({ field }) => {
              const isDisabled = !!ruangParam;

              const selectedRuangId = ruangList.find(
                (ruang: any) => String(ruang.id_ruang) === ruangParam
              )?.id_ruang;

              useEffect(() => {
                if (isDisabled && selectedRuangId) {
                  form.setValue("id_ruang", String(selectedRuangId));
                }
              }, [isDisabled, selectedRuangId]);
